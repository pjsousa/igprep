...
- Posts 1.3 & 1.4 — *False Sharing* and *Memory Barriers* (already applied in Exercise 04, now validated by profiling)
...
Remove your Exercise 04 `AtomicReferenceArray`-based ring buffer and replace it with a properly wired `Disruptor<PriceEvent>`.
...
The `AtomicReference<Map<String, PriceTick>>` copy-on-write cache from Exercise 04 still copies the entire map on every update. At 1,000,000 updates/second across 10,000 instruments, this is 1,000,000 map copies per second — unsustainable.
